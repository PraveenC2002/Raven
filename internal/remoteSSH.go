...
func (r *remoteSSH) execute(ctx context.Context, cmd string) (*sshOutput, error) {
	...
	case <-sshCtx.Done():
		if errors.Is(sshCtx.Err(), context.DeadlineExceeded) {
			r.emitUpdate("remote ssh command timed out")
			...
			r.client.Close()
			r.emitUpdate("remote ssh : context canceled")
...
func (r *remoteSSH) callTool(ctx context.Context, fc *llmFunctionCall) (*llmFunctionResponse, *agentErr) {
	...
	if err != nil {
		...
		remoteSSHLogger.Error("fc object marshal", "error", err.Error())
		remoteSSHLogger.Error("fc object marshal", "error", err.Error())
	...
	if err != nil {
		err = fmt.Errorf("remote ssh: unmarshal function args: %w", err)
		remoteSSHLogger.Error("fc object unmarshal", "error", err.Error())
		return newErrFr(fc, err), nil
	}

	r.emitUpdate("validating shell command")
	remoteSSHLogger.Info("validating shell command")
	err = r.bouncer.validate(fcObj)
	if err != nil {
		err = fmt.Errorf("remote ssh: validate command: %w", err)
		remoteSSHLogger.Warn("shell command validation", "warning", err.Error())
		return newErrFr(fc, err), nil
	}

	r.emitUpdate("constructing shell command")
	remoteSSHLogger.Info("constructing shell command")
	cmd, err := r.bouncer.constructCmd(fcObj)
	if err != nil {
		err = fmt.Errorf("remote ssh: construct command: %w", err)
		remoteSSHLogger.Error("shell command construction", "error", err.Error())
		return nil, &agentErr{kind: agentErrTerminate, err: err}
	}

	remoteSSHLogger.Info("executing shell command : ", cmd)
	out, err := r.execute(ctx, cmd)
	if err != nil {
		err = fmt.Errorf("remote ssh: execute %q: %w", cmd, err)
		remoteSSHLogger.Info("execute cmd", "error", err.Error())
		return nil, &agentErr{kind: agentErrTerminate, err: err}
	}
	...
	if err != nil {
		...
		remoteSSHLogger.Error("parse output", "error", err.Error())
		return nil, &agentErr{kind: agentErrFatal, err: err}
	}

	remoteSSHLogger.Block("Command output : ", buf.String())
	r.emitUpdate("returing shell command output")
	remoteSSHLogger.Info("returing shell command output")
